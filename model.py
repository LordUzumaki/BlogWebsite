...
class User(db.Model, UserMixin):  # Inherit from db.Model and UserMixin
    ...
    date_joined = db.Column(db.DateTime, nullable=False, default=datetime.now(timezone.utc))
...
class Post(db.Model):  # Inherit from db.Model
    ...
    date_posted = db.Column(db.DateTime, nullable=False, default=lambda: datetime.now(timezone.utc))  # Date posted column, default value is current time
